...
class sviMS(object):
    ...
    def run(self,indict):
        ...
        print(f"params: {params}\nrng key: {self.rng_key}\nbig array: {(settings.get('post_resample',int(settings.get('steps',30000)/3)),)}")
...
class sviTP(object):
    ...
    def run(self,indict):
        ...
        # reconstruct the posterior
        params = svi_result.params
        print(f"params: {type(params)}\nrng key: {type(self.rng_key)}\nbig array: {type((settings.get('post_resample',int(settings.get('steps',30000)/3)),)})")
