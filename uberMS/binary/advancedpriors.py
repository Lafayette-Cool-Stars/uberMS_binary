...
class Keplerian_Prior(distfn.Distribution):
    ...
    def __init__(self, q_low=1e-5, q_high=1.0, q=0.5, v1=2.3, v2=1.3, tolerance=1e-5, validate_args=None):
        """
        q \propto v_1 / v_2
        ----------
        ...
        v1 = numpyro.sample("v1", distfn.Uniform(0.5, 3.0))
        # numpyro.sample returns an array. Do i want self.v1 to be an array,
        # then check element-wise to make v1/v2 is proportional to q?

        # Check if q propto v1/v2 --> Think this actually gets done in the 
        # log_prob function
    ...
    def log_prob(self, q):
        ...
        # TODO: Check lax.cond syntax
        lnprior = lax.cond(q == self.v1 / self.v2, lnprior_high, lnprior_low, mass)
